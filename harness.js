// Minimal stubs to runtime-exercise drawBattleAntBody for every fighter type/side without a real DOM.
let battleAnimT = 1.234;
...
const UNITS = {
  ...
  medic:{color:'#e0688a'}, archer:{color:'#c9932f'}, siege:{color:'#8a7a5a'},
  zapper:{color:'#d9c22e'}, bomber:{color:'#7a3fb5'}, wasp:{color:'#d99a1e'}, monarch:{color:'#c04fb0'},
};
let callCount = 0;
function makeCtx(){
  const noop = () => callCount++;
  const handler = { get(t,p){ if(p in t) return t[p]; return function(){ callCount++; return undefined; }; } };
  const base = { save:noop, restore:noop, beginPath:noop, moveTo:noop, lineTo:noop, arc:noop, ellipse:noop,
    fill:noop, stroke:noop, translate:noop, fillText:noop, closePath:noop };
...
const types = ['soldier','spitter','guard','medic','archer','siege','zapper','bomber'];
let errors = 0;
for(const type of types){
  for(const side of ['me','en']){
    for(let i=0;i<5;i++){
      battleAnimT += 0.1;
      try{
        drawBattleAntBody({type, side, _wob:undefined});
      }catch(e){
        errors++;
        console.log('ERROR for', type, side, ':', e.message);
      }
    }
  }
}
console.log(errors===0 ? 'ALL TYPES OK, canvas calls made: '+callCount : (errors+' ERRORS'));
