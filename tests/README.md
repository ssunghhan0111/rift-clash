...
| `kidsafe_test.js` | The kids-mode safety gates, driven by a **hostile** raw-socket client that ignores the UI entirely: a kids room asked to be public comes back private and is never listed, a public room switched to kids goes private on the spot, free text is refused, quick chat only resolves ids from the table (a client-supplied string cannot ride along), `voiceJoin` is denied and the host cannot re-enable it — and Survival/Versus are provably unaffected by any of it |
